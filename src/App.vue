<template>
  <div class="container mx-auto p-4">
    <h1 class="text-6xl font-bold mb-16">Doctrinal Mastery Game</h1>
    <div v-if="gameState === 'setup'">
      <h2 class="text-2xl font-bold">Team Setup</h2>
      <div class="space-y-2 mt-4">
        ...
        >
          <span>{{ team }}</span>
          <button @click="removeTeam(team)" class="text-red-500">Remove</button>
        </div>
      </div>
      ...
      <button
        ...
        class="mt-4 bg-green-500 text-white px-4 py-2 rounded disabled:opacity-50"
      >
        Start Game
      </button>
    </div>
    <div v-else-if="gameState === 'play'" class="flex">
      <div class="w-3/4 pr-4">
        <div v-if="!selectedCategory" class="grid grid-cols-2 gap-4">
          <button
            ...
            :class="['h-32 text-white', categoryColors[column]]"
          >
            <strong class="text-xl">{{ column }}</strong>
            <br />
            {{ availableScriptures[column].length }} Remaining
          </button>
        </div>
        <div v-else>
          <div class="space-y-4">
            <div class="border p-4 rounded">
              <h3 class="font-bold border-b mb-4 pb-4 text-lg">
                Current Scripture
              </h3>
              <p class="text-left text-xl">{{ currentScripture.text }}</p>
            </div>
            <div class="p-4 border border-rounded">
              <h3 class="border-b pb-4 mb-4 text-lg font-bold">
                Available Options
              </h3>
      ...
      <div class="w-1/4">
        ...
        <div class="border p-4 rounded" v-if="selectedCategory">
          <h3 class="text-xl font-bold mb-2">Guessing Team</h3>
          <button
            v-for="(score, team) in scores"
            :key="team"
            class="flex justify-between text-center w-full mb-2 transision"
            :class="{ 'bg-green-500': team === guessingTeam }"
...
const categoryColors = {
  'New Testament': 'bg-blue-500 hover:bg-blue-600',
  'Old Testament': 'bg-green-500 hover:bg-green-600',
  'Book of Mormon': 'bg-yellow-500 hover:bg-yellow-600',
  'Doctrine and Covenants': 'bg-purple-500 hover:bg-purple-600',
};
